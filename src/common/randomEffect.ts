...
import { LAYER, RANDOM_EFFECT } from "../data/definition";
import { stageInfo } from "../data/stage";
import { g_CanvasWidth, g_InitStageWidth, g_PageHeight, g_PageWidth, g_StagePosX, g_StagePosY, g_StageWidth } from "../data/global";

const { sprite, add, pos, scale, area, z, anchor, loop, vec2, choose, chance, randi, offscreen, wait, play } = k;
...
export const startRandomEffect = (stage: number, randomEffect: RANDOM_EFFECT[]) => {
  ...
  const windowPos: Vec2[] = [
    vec2(g_StagePosX + 20, g_StagePosY + 30).scale(bkgScale),
    vec2(g_StagePosX + 250, g_StagePosY + 30).scale(bkgScale),
    vec2(g_StagePosX + 500, g_StagePosY + 30).scale(bkgScale),
    vec2(g_StagePosX + 760, g_StagePosY + 30).scale(bkgScale),
  ];
    ...
    loop(5, () => {
      if (isRainning == false) {
        ...
        windowPos.forEach((wp: Vec2) => {
          const weatherEffect = add([sprite("rain01"), pos(wp.x, wp.y), scale(bkgScale), anchor("topleft"), z(LAYER.OUTDOOR), "rain01"]);
          isRainning = true;
  ...
  if (randomEffect.includes("RE_THUNDER")) {
    ...
    loop(3, () => {
      if (chance(0.5)) {
        ...
        const weatherEffect = add([sprite("thunder"), pos(wp.x, wp.y), , scale(bkgScale), z(LAYER.OUTDOOR), "thunder"]);

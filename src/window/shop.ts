...
import { g_GameCoin } from "../data/global";
import { getGameItemCount, ITEM_TYPE, gameItem, IGameItem } from "../data/item";
import { g_StagePosX, g_StageHeight, g_StageWidth } from "../data/global";
...
export const shop = () => {
  ...
  add([sprite("shopWindow"), "Modal", anchor("center"), area(), pos(g_StageWidth / 2 + g_StagePosX, SHOP_Y_POS + g_StageHeight / 2), z(shopBgLayer)]);
  add([
    "Modal",
  ...
  add([sprite("close"), "Modal", anchor("center"), area(), pos(g_StageWidth / 2 + 150 + g_StagePosX, 50 + SHOP_Y_POS), z(shopUILayer), "close"]);
  ...
  const addItem = (ItemName: string, price: number, Y_pos: number, ItemType: ITEM_TYPE) => {
    ...
    add([sprite("buy"), "Modal", anchor("center"), area(), pos(BUTTON_X_POS, Y_pos), z(shopUILayer), "buy_button", "HoverCursor", ItemType]).add([
      "Modal",
      pos(vec2(0, 0)),
      anchor("center"),
      z(shopActionLayer),
